...
fn failed_transform_response(
    ...
    let message = match (&failure, thumbhash) {
        (TransformFailure::Failed(_), true) => "thumbhash transform failed",
        (TransformFailure::Panicked(_), true) => "thumbhash transform task panicked",
        (TransformFailure::Failed(_), false) => "image transform failed, serving raw origin bytes",
        (TransformFailure::Panicked(_), false) => {
            "image transform task panicked, serving raw origin bytes"
        }
    };
    let cause: &dyn std::fmt::Display = match &failure {
        TransformFailure::Failed(e) => *e,
        TransformFailure::Panicked(e) => *e,
    };
    tracing::warn!(error = %cause, path = %path, transform = %transform, "{message}");
    if !thumbhash {
        return handle_transform_failure(state, req, tp, raw_data, if_none_match);
    }
    match failure {
        TransformFailure::Failed(e) => thumbhash_failure_response(Some(e)),
        TransformFailure::Panicked(_) => thumbhash_failure_response(None),
    }
}
...
mod tests {
    ...
    /// An app whose origin serves 12 bytes of text labelled `image/jpeg`,
    /// so every transform of it fails.
    async fn state_with_corrupt_jpeg_origin() -> Arc<AppState> {
        init_vips();
        let mut cfg = Config::defaults();
        cfg.origin.base_url = serve_repeatedly(CORRUPT_JPEG_ORIGIN).await;
        Arc::new(AppState::new(cfg))
    }
    ...
    async fn thumbhash_failure_returns_422_json_envelope_not_origin_bytes() {
        let router = build_router(state_with_corrupt_jpeg_origin().await);
        let (status, headers, body) =
    ...
    async fn thumbhash_failure_is_never_cached() {
        let state = state_with_corrupt_jpeg_origin().await;
        let router = build_router(Arc::clone(&state));
    ...
    #[test]
    fn thumbhash_failure_response_maps_each_error_kind_to_its_status() {
