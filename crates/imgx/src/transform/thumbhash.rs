...
//! The encoder uses f64 in the reference operation order and calls `libm::cos`,
...
pub fn rgba_from_bands(
    ...
    for sample in bytes.chunks_exact(bands) {
        match *sample {
            ...
            // Cannot happen: `chunks_exact(bands)` yields 1 to 3 samples here. INV-17 forbids a panic.
            _ => return Err(ThumbhashError::UnsupportedBands(bands as u32)),
        }
    }
    Ok(rgba)
}
