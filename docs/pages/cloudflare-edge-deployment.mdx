...
**Not related to `imgx.pages.dev`.** This repository's documentation site is deployed to Cloudflare Pages via `.github/workflows/docs.yml`, which runs `wrangler pages deploy docs/dist --project-name=zimgx` on every push to `main`. That is a completely separate deployment (a static docs site, not an image proxy) with its own project name and its own CI job. `imgx-edge`'s `wrangler.toml` has no `routes`/zone binding configured — which hostname it should front is left as an open decision, not assumed to be `imgx.pages.dev` or any other existing Cloudflare Pages project.
