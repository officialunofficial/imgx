...
### `draw` (overlays) — not yet enabled

`draw.<N>.url`, `draw.<N>.width`/`.height`, `.top`/`.left`/`.bottom`/`.right`, `.opacity`, `.repeat`, `.background`, `.rotate`, `.fit`, `.gravity` parse and validate (an array of overlays, indexed by `<N>` starting at `0`), but **every request naming a `draw` overlay currently returns `422`**: the remote-URL fetch needed to actually retrieve an overlay image is deliberately not implemented yet, for the same SSRF-related reasons imgx doesn't fetch arbitrary remote main-image sources. See `docs/CLOUDFLARE_PARITY.md` gap 11 for the full status and scope-decision rationale.

```
/cdn-cgi/image/draw.0.url=https://example.com/logo.png,draw.0.opacity=0.5/photo.jpg
→ 422 Unprocessable Entity (draw overlays not yet enabled)
```
