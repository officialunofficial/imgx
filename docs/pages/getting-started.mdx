...
This produces a binary at **target/release/imgx**.
...
export IMGX_ORIGIN_BASE_URL=https://my-bucket.s3.amazonaws.com
```
...
export IMGX_R2_BUCKET_ORIGINALS=my-images
```
...
curl http://localhost:8080/cdn-cgi/image/photos/hero.jpg

# Resize to 400px wide
curl http://localhost:8080/cdn-cgi/image/w=400/photos/hero.jpg

# Resize and convert to WebP at quality 85
curl http://localhost:8080/cdn-cgi/image/w=800,f=webp,q=85/photos/hero.jpg
```
...
export IMGX_CACHE_DEFAULT_TTL_SECONDS=3600   # 1 hour Cache-Control
```

imgx generates ETags automatically. Clients that send `If-None-Match` receive `304 Not Modified` responses, saving bandwidth.
...
- [Migrating from Cloudflare](/migrating-from-cloudflare) — Replace imagedelivery.net with imgx
