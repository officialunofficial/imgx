...
- **Errors:** a failed request returns the JSON error envelope. A source over `IMGX_TRANSFORM_MAX_PIXELS`, or a source that libvips cannot process, returns `422`. An internal failure returns `500`. imgx never returns the raw origin bytes for this format. It ignores `onerror=redirect`, and it never caches a failure.
