...
- **`draw`:** a request that combines `format=thumbhash` with `draw` overlays returns `422`. The error detail says that draw overlays cannot be combined with `format=thumbhash`. The check runs before the draw permission check and before imgx fetches any overlay.
...
A `smart` or `attention` crop with `fit=cover`, `fit=crop`, or `fit=aspect-crop` decodes the source at full size. The content search then reads the same pixels as it always did. A resize with another gravity decodes a JPEG or WebP source at reduced size, which uses less memory.
:::
...
imgx caches transform results under a deterministic key. The key holds the image path, the transform parameters, and a format segment.

**Key format:** `<image-path>|<transform-string>|<format>`

**Example:** `photos/hero.jpg|w=400,h=300|webp`

With an explicit format, the format segment is that format's name. With `auto`, the segment is `auto:` followed by the formats that the `Accept` header supports, joined by `+`, in the order `avif`, `webp`, `jpeg`, `png`, `gif`. For example, `Accept: image/jpeg` gives `auto:jpeg`, and `Accept: */*` gives `auto:avif+webp+jpeg+png+gif`. Two `Accept` headers that support the same formats share one cache entry. Two headers that support different formats never share one. A client therefore always gets a format that it accepts.

**Example:** `photos/hero.jpg|w=400,h=300|auto:avif+webp`
