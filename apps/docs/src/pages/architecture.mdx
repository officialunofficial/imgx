...
flowchart TD
    ...
    B -->|"Parse the target path. Detect well-known routes (/health, /ready, /metrics) or extract image path and transform string."| C[Params Parser]
    C -->|"Parse comma-separated key=value pairs into a TransformParams struct. Validate ranges for w, h, q, fit, g, and others."| D[Cache Lookup]
    D -->|"hit → serve with ETag and Cache-Control"| R["Response<br/>200 with image body, ETag, Cache-Control, Vary: Accept"]
    D -->|"miss"| E[Origin Fetch]
    E -->|"HTTP GET to base URL + path, or S3 GET from the R2 originals bucket."| F["Transform Pipeline (libvips)"]
    F -->|"1. Probe (detect animation, load first frame) 2. Decide and reload (animated or static path) 3. Resize → Effects → Encode"| G[Cache Store]
    G -->|"Write to L1 (memory) and L2 (R2) if configured."| R
```
...
└── imgx/                          The binary. forbid(unsafe_code) -- everything unsafe lives in imgx-vips.
    └── src/
        ...
        │   └── thumbhash.rs       ThumbHash encoder, RGBA expansion, and base64 text encoder.
        │
        ├── http/
...
```mermaid
flowchart TD
    subgraph TieredCache
        L1["L1: Memory (MemoryCache)<br/>- LRU evict<br/>- Size-bound<br/>- Fastest"]
        L2["L2: R2/S3 (R2Cache)<br/>- Persistent<br/>- Cross-restart<br/>- Shared"]
    end
```
