...
| `IMGX_TRANSFORM_AVIF_EFFORT` | `u8` | `4` | AVIF encoder effort, `0` to `9`. A lower value encodes faster and writes larger files. The default is the libvips default. The cache key does not include this setting. A cached variant keeps the effort it had when imgx encoded it. |
