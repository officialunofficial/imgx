...
curl -sf "http://${HOST}/image/${IMAGE}" > /dev/null 2>&1 || true
curl -sf "http://${HOST}/image/w=800,h=600,f=jpeg,q=80/${IMAGE}" > /dev/null 2>&1 || true
curl -sf "http://${HOST}/image/w=800,h=600,f=webp,q=80/${IMAGE}" > /dev/null 2>&1 || true
curl -sf "http://${HOST}/image/w=400,h=300,f=webp,q=80/${IMAGE}" > /dev/null 2>&1 || true
curl -sf "http://${HOST}/image/w=200,h=150,f=webp,q=80/${IMAGE}" > /dev/null 2>&1 || true

echo "Cache hits (primed):"
run_bench "original (no transform)" "image/${IMAGE}"
run_bench "800x600 JPEG q80" "image/w=800,h=600,f=jpeg,q=80/${IMAGE}"
run_bench "800x600 WebP q80" "image/w=800,h=600,f=webp,q=80/${IMAGE}"
run_bench "400x300 WebP q80" "image/w=400,h=300,f=webp,q=80/${IMAGE}"
run_bench "200x150 WebP q80 (thumbnail)" "image/w=200,h=150,f=webp,q=80/${IMAGE}"
